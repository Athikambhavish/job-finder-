  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F9F8F4', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#fff', borderRadius: 20, padding: 40, width: '100%', maxWidth: 420, border: '1.5px solid #ECEAE4', boxShadow: '0 8px 40px rgba(0,0,0,.08)' }}>
        ...
        <p style={{ fontSize: 13, color: '#888', marginBottom: 24 }}>Join thousands finding their dream jobs</p>
        ...
        {[
          ...
          ['Location (optional)', 'location', 'text', 'e.g. New York, NY'],
        ...
        <button onClick={handleSubmit} disabled={loading || !valid}
          style={{ width: '100%', padding: 13, marginTop: 8, background: valid ? '#0F4C8A' : '#C8D8F0', color: '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: valid ? 'pointer' : 'not-allowed', fontFamily: 'inherit' }}>
          {loading ? 'Creating account…' : 'Create Account'}
        </button>

        <p style={{ textAlign: 'center', fontSize: 13, color: '#888', marginTop: 20 }}>
          Already have an account?{' '}
          <Link to="/login" style={{ color: '#0F4C8A', fontWeight: 600, textDecoration: 'none' }}>Sign in</Link>
        </p>
      </div>
    </div>
  )
